...
const TotalVolume = ({ coinId }) => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        for (let i = 0; i < res.data.total_volumes.length; i++) {
          let price = res.data.total_volumes[i][1];

          dataArray.push({
            date: new Date(res.data.total_volumes[i][0]).toLocaleDateString(),
            TotalVolumes: price < "50" ? price : parseInt(price),
          });
        }
